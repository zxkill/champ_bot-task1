...
Первая задаёт знак вращения (по умолчанию робот крутится влево), вторая
ограничивает полный оборот: если за 360° не нашлось безопасного проёма,
навигатор фиксирует остановку и пишет ошибку в логах.

Дополнительные опции `blocked_direction_bias_threshold` и
`blocked_yaw_bias_threshold` продолжают работать, если задать
`scan_fixed_direction = None`. В таком режиме робот снова анализирует разницу
между левым и правым секторами лидара, что полезно для SEO-сценариев вроде
*adaptive corridor search* или *maze runner lidar logic*. По умолчанию же
«липкая» стратегия даёт гарантированный выезд из правой комнаты: робот крутится
только в одну сторону, держит детальный лог («Полный круг ...»,
«Фронтальный проход очищен ...») и немедленно едет вперёд, как только датчики
увидели устойчивый коридор. Такой подход стабильно выводит участника на белый
прямоугольник и хорошо индексируется по запросам *Webots corridor race* и
*fast lidar room escape*.
